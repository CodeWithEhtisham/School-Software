...
class DBHandler:
    def __init__(self):
        ...
        # self.conn.execute("CREATE TABLE IF NOT EXISTS account_details (account_details_id INTEGER PRIMARY KEY AUTOINCREMENT, account_id INTEGER, date TEXT, refrence TEXT DEFAULT 'account created', description TEXT, cash_in REAL DEFAULT 0, cash_out REAL DEFAULT 0, remaining REAL,roznamcha_id, FOREIGN KEY(account_id) REFERENCES accounts(accounts_id),FOREIGN KEY(roznamcha_id) REFERENCES roznamcha(roznamcha_id))")
        # self.create_table('business', 'id INTEGER PRIMARY KEY AUTOINCREMENT, business_name TEXT, business_email TEXT, business_address TEXT, business_contact TEXT, business_owner TEXT')
        ...
        # self.conn.execute("CREATE TABLE IF NOT EXISTS roznamcha (roznamcha_id INTEGER PRIMARY KEY AUTOINCREMENT, khata_id INTEGER, accounts_id INTEGER, date TEXT,cash_type TEXT, refrences TEXT, description TEXT,cash_in INTEGER DEFAULT 0, cash_out INTEGER DEFAULT 0,remaining INTEGER,accounts_remaining INTEGER DEFAULT 0, FOREIGN KEY(khata_id) REFERENCES khata(khata_id), FOREIGN KEY(accounts_id) REFERENCES accounts(accounts_id))")

...
class PayFeeWindow(QMainWindow, FORM_MAIN):
    def __init__(self, reg_no):
        ...
        self.setupUi(self)
        self.db = DBHandler()
        std = self.db.select(table_name='students', columns='id,name,remaining_fee',
                             condition=f"addmission_no = '{reg_no}'")[0]
        self.std_id = std[0]
        ...
        try:
            ...
            if discount >= 0:
                ...
                if total_remaining >= 0:
                    ...
                    QMessageBox.warning(
                        self, "Warning", "Discount cannot be greater than remaining fee")
                    self.txt_discount.setText("")
                    self.txt_total_remaining.setText(str(remaining_fee))
            else:
                self.txt_total_remaining.setText(
                    str(remaining_fee-float(paid_fee)))
        ...
        try:
            ...
            if pay_fee >= 0:
                ...
                if total_remaining >= 0:
                    ...
                    QMessageBox.warning(
                        self, "Warning", "Paid fee cannot be greater than remaining fee")
    ...
    def pay_fee(self):
        try:
            ...
            if paid_fee and date and challan_no and description:
                ...
                if fee_id:
                    ...
                    QMessageBox.information(
                        self, 'Success', 'Fee Paid Successfully')
                    self.close()
                else:
            ...
            QMessageBox.information(
                self, 'Error', f'Please Enter Valid Data {e}')
